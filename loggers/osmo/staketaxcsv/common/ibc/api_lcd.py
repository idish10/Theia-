...
import requests
import pattrn_robust_tracking.loggers.osmo.staketaxcsv.common.ibc.constants as co
from pattrn_robust_tracking.loggers.osmo.staketaxcsv.common.debug_util import use_debug_files
from pattrn_robust_tracking.loggers.osmo.staketaxcsv.common.ibc.api_common import (
...
from pattrn_robust_tracking.loggers.osmo.staketaxcsv.settings_csv import REPORTS_DIR
...
class LcdAPI:
    ...
    def _query(self, uri_path, query_params, sleep_seconds=0):
        ...
        print("Requesting url {}?{} ...", url, urlencode(query_params))
...
def get_txs_all(node, wallet_address, limit=TXS_LIMIT_PER_QUERY, sleep_seconds=1,
                debug=False, events_types=None):
    LcdAPI.debug = debug
    api = LcdAPI(node)
    max_txs = 20000
